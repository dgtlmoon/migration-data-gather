...
- `calculate_remaining_discount_cycles(subscription)`: Converts a coupon's duration (expressed in months by Stripe) into the number of billing cycles still to be discounted.
...
- `discount_remaining_cycles` counts the billing cycles **still to be discounted after migration**; the current period has already been invoiced by Stripe and is not counted. A `forever` coupon exports as blank, and a `once` coupon as `0`. Confirm with your Solutions Engineer that blank is the value Paddle expects for a discount that never expires.
